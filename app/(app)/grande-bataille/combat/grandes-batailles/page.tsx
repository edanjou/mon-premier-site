...
import {
  createGrandeBataille,
  deleteGrandeBataille,
  listGrandeBatailles,
  updateGrandeBataille,
  type GrandeBataille,
  type GrandeBatailleInput,
} from "@/lib/grande-batailles";
...
const CHAPTER_SLOT_LABELS = ["Chapitre 1", "Chapitre 2", "Chapitre 3"];

function formatGrandeBatailleDate(dateStr: string): string {
...
function GrandeBatailleModal({
  initial,
  onClose,
  onSaved,
}: {
  initial?: GrandeBataille;
  onClose: () => void;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: GrandeBatailleInput = { name, date };

    try {
      if (initial) {
        await updateGrandeBataille(initial.id, payload);
      } else {
        await createGrandeBataille(payload);
      }
      await onSaved();
      onClose();
    } catch {
    ...
    >
      ...
      >
        <h2 className="font-semibold text-foreground">
          {initial ? "Modifier la Grande Bataille" : "Créer une Grande Bataille"}
        </h2>
        <input
          type="text"
          required
...
function GrandeBatailleChapterForm({
  grandeBatailleId,
  slotLabel,
  initial,
  position,
  ...
  const [title, setTitle] = useState(initial?.title ?? slotLabel);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: GrandeBatailleChapterInput = {
      grande_bataille_id: grandeBatailleId,
      title,
...
function GrandeBatailleChaptersModal({
  grandeBataille,
  canWrite,
  onClose,
}: {
  grandeBataille: GrandeBataille;
  canWrite: boolean;
  onClose: () => void;
}) {
  ...
  const fetchChapters = async () => {
    const data = await listGrandeBatailleChapters(grandeBataille.id);
    setChapters(data);
  };
  ...
  const chapterForSlot = (position: number) =>
    ...
    >
      <div
        ...
        className={`flex max-h-[85vh] w-full flex-col gap-4 overflow-y-auto rounded-2xl bg-white p-6 shadow-lg dark:bg-zinc-900 ${
          editingSlot !== null ? "max-w-5xl" : "max-w-2xl"
        }`}
      >
        {editingSlot === null ? (
          <>
            <h2 className="font-semibold text-foreground">
              Chapitres — {grandeBataille.name}
            </h2>
            ...
            {!isLoading && (
              <div className="flex flex-col gap-2">
                {CHAPTER_SLOT_LABELS.map((slotLabel, position) => {
                  const chapter = chapterForSlot(position);
                  return (
                    <div
                      key={position}
          ...
          <GrandeBatailleChapterForm
            grandeBatailleId={grandeBataille.id}
            slotLabel={CHAPTER_SLOT_LABELS[editingSlot]}
            initial={chapterForSlot(editingSlot)}
...
export default function GrandesBataillesPage() {
  const [grandeBatailles, setGrandeBatailles] = useState<GrandeBataille[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [editingGrandeBataille, setEditingGrandeBataille] =
    useState<GrandeBataille | null>(null);
  const [chaptersGrandeBataille, setChaptersGrandeBataille] =
    useState<GrandeBataille | null>(null);
  ...
  const fetchGrandeBatailles = async () => {
    setIsLoading(true);
    setError(null);
    try {
      const data = await listGrandeBatailles();
      setGrandeBatailles(data);
    } catch {
      setError("Impossible de charger les Grandes Batailles.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- fetchGrandeBatailles sets a loading flag ahead of an async fetch
    fetchGrandeBatailles();
  }, []);

  const handleDelete = async (grandeBataille: GrandeBataille) => {
    if (!canWrite) return;
    if (
      !window.confirm(
        `Supprimer la Grande Bataille "${grandeBataille.name}" ?`,
      )
    )
      return;
    try {
      await deleteGrandeBataille(grandeBataille.id);
      setGrandeBatailles((prev) =>
        prev.filter((g) => g.id !== grandeBataille.id),
      );
    } catch {
  ...
  const visibleGrandeBatailles = grandeBatailles
    .filter(
      (g) =>
        !query ||
        g.name.toLowerCase().includes(query) ||
        formatGrandeBatailleDate(g.date).toLowerCase().includes(query),
    )
    .sort((a, b) => {
  ...
  const {
    page: grandeBataillesPage,
    pageCount: grandeBataillesPageCount,
    setPage: setGrandeBataillesPage,
    pageItems: pagedGrandeBatailles,
  } = usePagination(visibleGrandeBatailles);
      ...
      <div>
            ...
            <>
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                ...
                {canWrite && (
                  ...
                  >
                    <Plus size={16} />
                    Créer une Grande Bataille
                  </button>
                )}
              </div>

              {grandeBatailles.length === 0 && (
                <p className="text-sm text-foreground/60">
                  Aucune Grande Bataille pour l&apos;instant.
                </p>
              )}

              {grandeBatailles.length > 0 &&
                visibleGrandeBatailles.length === 0 && (
                  <p className="text-sm text-foreground/60">
                    Aucune Grande Bataille ne correspond à ces critères.
                  </p>
                )}
              ...
              {visibleGrandeBatailles.length > 0 && (
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[500px] text-left text-sm">
                    ...
                    <tbody>
                      {pagedGrandeBatailles.map((grandeBataille) => (
                        <tr
                          key={grandeBataille.id}
                        ...
                        >
                          ...
                          <td className="py-2 pr-4 text-foreground/80">
                            {formatGrandeBatailleDate(grandeBataille.date)}
                          </td>
                          <td className="py-2 pr-4">
                            <div className="flex items-center justify-between gap-1">
                              <button
                                onClick={() =>
                                  setChaptersGrandeBataille(grandeBataille)
                                }
                                className="flex items-center gap-1.5 rounded-full border border-black/[.08] px-3 py-1.5 text-xs font-medium text-foreground/70 transition-colors hover:bg-black/[.04] dark:border-white/[.145] dark:hover:bg-white/[.08]"
                              >
                                <BookText size={14} />
                              ...
                              {canWrite && (
                                <div className="flex items-center gap-1">
                                  <button
                                    onClick={() =>
                                      setEditingGrandeBataille(grandeBataille)
                                    }
                                    aria-label="Modifier"
                  ...
                  <Pagination
                    page={grandeBataillesPage}
                    pageCount={grandeBataillesPageCount}
                    onPageChange={setGrandeBataillesPage}
                  />
                </div>
              )}
            </>
        ...
        {isCreateOpen && (
          <GrandeBatailleModal
            ...
            onSaved={fetchGrandeBatailles}
          />
        )}
        {editingGrandeBataille && (
          <GrandeBatailleModal
            initial={editingGrandeBataille}
            onClose={() => setEditingGrandeBataille(null)}
            onSaved={fetchGrandeBatailles}
          />
        )}
        {chaptersGrandeBataille && (
          <GrandeBatailleChaptersModal
            grandeBataille={chaptersGrandeBataille}
            canWrite={canWrite}
            onClose={() => setChaptersGrandeBataille(null)}
          />
        )}
      </div>
    </RequireFeature>
